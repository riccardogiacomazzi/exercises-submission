...
const App = () => {
  ...
  const addEntry = (event) => {
    ...
    if (allNames.includes(newName)) {
      ...
      const entryObject = {
        name: newName,
        number: newNumber,
        // id: Math.max(...persons.map((id) => id.id)) + 1, ---> ID is chosen in the backend
      };
      ...
      numberService
        ...
        .catch((error) => {
          ...
          setNotification(`Error: missing "name" or "number"`);
          setTimeout(() => {
            setNotification(null);
          }, 5000);
        });
  ...
  const handleClickDelete = (id) => {
    ...
    if (window.confirm(`Do you want to delete "${toDeleteName.map((index) => index.name)}"?`)) {
      numberService
        .deleteNumber(id)
        .then(() => setNotificationStyle("notificationSuccess"))
        .catch((error) => {
          setNotificationStyle("notificationError");
          setNotification(
